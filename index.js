...
app.get("/", (_req, res) => {
  res.json({ message: "🔥 Crave Restaurant API is running!", version: "2.0.0" });
});
...
app.get("/api/menu", async (req, res) => {
  try {
    ...
    `;
    const params = [];
    if (category && category !== "all") { query += " AND c.id = ?"; params.push(parseInt(category)); }
    if (sortPrice === "low-to-high") query += " ORDER BY m.price_bdt ASC, c.display_order ASC, m.display_order ASC";
    else if (sortPrice === "high-to-low") query += " ORDER BY m.price_bdt DESC, c.display_order ASC, m.display_order ASC";
...
app.put("/api/admin/menu/:id", verifyToken, requireRole("admin"), async (req, res) => {
  const { id } = req.params;
  const { price_bdt, is_available } = req.body;
  try {
    const updates = [];
    const params = [];
    if (price_bdt !== undefined) { updates.push("price_bdt = ?"); params.push(parseFloat(price_bdt)); }
...
app.post("/api/staff/login", async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, token, staff: { id: staff.id, name: staff.name, email: staff.email, role: staff.role } });
  } catch (error) {
...
app.get("/api/staff/me", verifyToken, async (req, res) => {
  res.json({ success: true, staff: { id: req.user.id, name: req.user.name, email: req.user.email, role: req.user.role } });
});

app.get("/api/admin/staff", verifyToken, requireRole("admin"), async (_req, res) => {
  try {
    const [rows] = await pool.query(
...
app.post("/api/admin/staff", verifyToken, requireRole("admin"), async (req, res) => {
  const { name, email, password, role } = req.body;
  if (!name || !email || !password || !role) {
    return res.status(400).json({ success: false, message: "All fields required" });
  }
  if (!["chef", "waiter"].includes(role)) {
    return res.status(400).json({ success: false, message: "Role must be 'chef' or 'waiter'" });
  }
  try {
    ...
    res.status(201).json({ success: true, message: "Staff added successfully", id: result.insertId });
  } catch (error) {
...
app.put("/api/admin/staff/:id/toggle", verifyToken, requireRole("admin"), async (req, res) => {
  const { id } = req.params;
  try {
    await pool.query("UPDATE staff SET is_active = NOT is_active WHERE id = ?", [id]);
...
app.delete("/api/admin/staff/:id", verifyToken, requireRole("admin"), async (req, res) => {
  const { id } = req.params;
  try {
...
app.post("/api/orders/create", async (req, res) => {
  ...
  try {
    ...
    let totalAmount = 0;
    for (const item of items) totalAmount += item.price_per_unit * item.quantity;
    ...
    for (const item of items) {
      await conn.query(
        `INSERT INTO order_items (order_id, menu_item_id, item_name, category_name, quantity, price_per_unit, subtotal, special_notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [orderId_db, item.menu_item_id, item.item_name, item.category_name || "",
          item.quantity, item.price_per_unit, item.price_per_unit * item.quantity, item.special_notes || null]
      );
    }
    ...
    res.status(201).json({
      ...
      order: { order_id: orderId, db_id: orderId_db, table_number, total_amount: totalAmount, status: "pending" }
    });
  } catch (error) {
    await conn.rollback();
...
app.get("/api/orders/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const [tracking] = await pool.query(
      "SELECT * FROM order_tracking WHERE order_id = ? ORDER BY timestamp DESC", [id]
    );
...
app.put("/api/orders/:id/status", verifyToken, async (req, res) => {
  ...
  if (role === "chef" && !["preparing", "ready"].includes(status)) {
    return res.status(403).json({ success: false, message: "Chefs can only set preparing or ready" });
  }
  if (role === "waiter" && !["served"].includes(status)) {
    return res.status(403).json({ success: false, message: "Waiters can only set served" });
  }
  ...
  try {
    ...
    await conn.commit();
    res.json({ success: true, message: "Order status updated", new_status: status });
  } catch (error) {
    await conn.rollback();
...
app.get("/api/admin/dashboard", verifyToken, requireRole("admin"), async (_req, res) => {
  try {
    const [todayOrders] = await pool.query("SELECT COUNT(*) as count FROM orders WHERE DATE(created_at) = CURDATE()");
    const [todayRevenue] = await pool.query("SELECT SUM(total_amount) as total FROM orders WHERE DATE(created_at) = CURDATE() AND status != 'cancelled'");
    ...
    const [staffCount] = await pool.query("SELECT role, COUNT(*) as count FROM staff WHERE is_active = TRUE GROUP BY role");
    ...
    res.json({
      ...
      data: {
        ...
        orders_by_status: byStatus.reduce((acc, r) => { acc[r.status] = r.count; return acc; }, {}),
        recent_orders: recentOrders,
        staff_by_role: staffCount.reduce((acc, r) => { acc[r.role] = r.count; return acc; }, {})
      }
    });
  } catch (error) {
...
app.get("/api/tables", async (_req, res) => {
  try {
    const [rows] = await pool.query("SELECT * FROM restaurant_tables WHERE is_active = TRUE ORDER BY table_number");
    res.json({ success: true, data: rows });
  } catch (error) {
...
app.post("/api/reservations", async (req, res) => {
  ...
  if (!customer_name || !customer_phone || !number_of_guests || !reservation_date || !reservation_time) {
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO reservations (customer_name, customer_email, customer_phone, number_of_guests, reservation_date, reservation_time, special_requests)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [customer_name, customer_email || null, customer_phone, number_of_guests, reservation_date, reservation_time, special_requests || null]
    );
    res.status(201).json({ success: true, message: "Reservation submitted!", reservation_id: result.insertId });
  } catch (error) {
...
app.get("/api/reservations", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (date) { query += " AND DATE(reservation_date) = ?"; params.push(date); }
...
app.put("/api/admin/reservations/:id/status", verifyToken, requireRole("admin"), async (req, res) => {
  const { id } = req.params;
  const { status } = req.body;
